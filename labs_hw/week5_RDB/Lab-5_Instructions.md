...
2. **Set up data source**
    ...
    2. Right click on `crsp_daily` in the canvas, and select "Convert to Custom SQL".
        ![Screenshot](figures/tableau_custom_sql.png "Screenshot: Convert to Custom SQL")
    4. In the SQL editor that pops up, enter the following code to filter for selected tickers:
        ```sql
        SELECT *
        FROM crsp_daily 
        WHERE ticker IN ('AAPL', 'MSFT', 'GOOGL')
        ```
    5. Select "Extract" in the top right, so that Tableau downloads the data to your Workbook and you won't have to reconnect to the database every time you open it
    6. Click "Sheet 1" at the bottom to go create the extract (which will take a few seconds, 25 in my case)

3. **Create Chart 1: Daily returns**
    1. Create line chart of `Ret` and `Date` (set to "Exact Date"), using `Ticker` for color. Hopefully this is becoming a familiar routine.

4. **Create Chart 2: Daily volume**
    1. Duplicate the Daily Returns worksheet
    2. Replace `Ret` with `Vol` on the Rows shelf

5. **Create Chart 3: Monthly returns**
